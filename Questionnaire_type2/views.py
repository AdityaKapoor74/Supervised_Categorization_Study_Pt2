...
def register(request,num):
    if num>4:
        return render(request,'Questionnaire_type2/set_404.html')
    request.session['setnumber'] = num
    return render(request,'Questionnaire_type2/register.html')

def contact(request):
    return render(request,'Questionnaire_type2/contact.html')

def about(request):
    return render(request,'Questionnaire_type2/about.html')

def terms(request):
    return render(request,'Questionnaire_type2/terms.html')
        ...
        if request.POST['firstname'] and request.POST['lastname'] and request.POST['email'] and request.POST['city'] and request.POST['country'] and request.POST['age']:
            ...
            try:
                ...
                try:
                    ...
                    return render(request,'Questionnaire_type2/register.html',{'error':'Please verify your email.'})
                user.email=email
                ...
                if 'terms' not in request.POST:
                    return render(request, 'Questionnaire_type2/register.html', {'error': 'Please accept the terms and conditions.'})
                ...
                return render(request,'Questionnaire_type2/register.html',{'error':'Incorrect values.Please try again.'})

            return render(request,'Questionnaire_type2/welcome.html')
        else:
            return render(request,'Questionnaire_type2/register.html',{'error':'All fields are required.'})
    else:
        return render(request,'Questionnaire_type2/register.html')

def training_phase_start_type3(request):
    return render(request, 'Questionnaire_type2/training_phase_start.html')

def observe_and_learn_type3(request):
    return render(request, 'Questionnaire_type2/observe_and_learn.html',
                  {'iteration': request.session['iteration']})

def observe_and_learn_instructions_type3(request):
    return render(request,'Questionnaire_type2/observe_and_learn_instructions.html')
...
def observe_and_learn_display_stimuli_type3(request):
    if len(request.session['obs_learn_samples'])!=0:
        ...
        return render(request, 'Questionnaire_type2/observe_and_learn_samples.html',{'samples':samples})

    else:
        if request.session['flag_training'] == True:
            return render(request,'Questionnaire_type2/classify_and_learn.html',{'iteration': request.session['iteration']})
        if request.session['setnumber'] == 0:
...
def fixation_screen_observe_type3(request):
    return render(request,'Questionnaire_type2/fixation_screen.html')

def classify_and_learn_instructions_type3(request):
    return render(request,'Questionnaire_type2/classify_and_learn_instructions.html')
...
def classify_and_learn_display_stimuli_type3(request):
    if request.method=="POST":
        ...
        if option==request.session['correct_answer']:
            ...
            return render(request, "Questionnaire_type2/correct_ans_classify.html",
                          {'time_taken': round(request.session['elapsed_time'], 2)})
        else:
            return render(request,"Questionnaire_type2/wrong_ans_warning.html",{'correct_answer':request.session['correct_answer'],'time_taken': round(request.session['elapsed_time'], 2)})
    ...
    if len(request.session['classify_learn_samples'])!=0:
        ...
        return render(request, 'Questionnaire_type2/classify_and_learn_samples.html',{'samples':samples})

    else:
        if request.session['flag_test'] == True:
            request.session['performance']+=str(request.session['score']*10)+"% "
            return render(request,'Questionnaire_type2/classify_result.html',{"performance":request.session['score']*10,"correct":request.session['score'],"wrong":10-request.session['score']})
        if request.session['setnumber'] == 0:
...
def fixation_screen_classify_type3(request):
    return render(request, 'Questionnaire_type2/fixation_screen_classify.html')
...
def classify_performance_type3(request):
    ...
    if request.session['result']<2:
        ...
        if request.session['iteration'] > 10:
            return render(request,"Questionnaire_type2/early_exit.html")
...
def common_features_test_block_display_stimuli_type3(request):
    if request.method=="POST":
        ...
        user_response.save()
        common_feature.save()
        return render(request,"Questionnaire_type2/selected_option.html",{'correct_answer':request.session['correct_answer'],"timetaken":round(request.session['elapsed_time'],2)})
    ...
    if len(request.session['common_features_test_samples'])!=0:
        ...
        request.session['start_time'] = time.time()
        return render(request, 'Questionnaire_type2/common_features_test_samples.html',{'samples':samples})

    else:
        if request.session['common_features_test_phase_flag'] == True and request.session['common_features_iteration']<4:
            request.session['common_features_test_phase_flag'] = False
            return render(request, "Questionnaire_type2/break_common_features.html")
        if request.session['common_features_test_phase_flag'] == True and request.session['common_features_iteration']>3:
            return render(request,"Questionnaire_type2/description.html")
    ...
    if request.method == "POST":
        try:
            ...
            if len(desc) != 0:
                ...
                return render(request, 'Questionnaire_type2/description.html', {'error': 'Please fill in the description'})

        except ValueError as e:
            return render(request, 'Questionnaire_type2/description.html', {'error': 'Please fill in the description'})

    return render(request, 'Questionnaire_type2/thankyou.html')
